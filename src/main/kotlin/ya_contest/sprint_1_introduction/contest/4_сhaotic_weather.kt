...
private fun main() {
    val countDays = readLine()!!.toInt()
    val arrayTemps = readLine()!!.split(" ").map { it.toInt() }

    val chaosFactor = countChaoticDays(countDays, arrayTemps)
    println(chaosFactor)
}

private fun countChaoticDays(size: Int, arr: List<Int>): Int {
    if (size == 1) return 1

    var counter = 0
    for (index in arr.indices) {
        if (index != 0 && index != size - 1 && arr[index] > arr[index - 1] && arr[index] > arr[index + 1]) {
            ++counter
            continue
        }
